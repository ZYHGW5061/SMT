...
{
    public partial class FrmUserMangement : DevExpress.XtraEditors.XtraForm
    {
        public FrmUserMangement()
        {
            InitializeComponent();
        ...
        {
            _selUser = gridView1.GetRow(e.RowHandle) as UserInfos;
        }
            ...
            if (_selUser != null)
            {
                if (WarningBox.FormShow("Verification", "Are you sure to delete the user？", "Warning!") == 1)
                {
                    ...
                    if (UserManager.Instance.DeleteUser(userName))
                    {
                        ...
                        LogRecorder.RecordUserOperationLog($"删除用户:{Name}", WestDragon.Framework.BaseLoggerClsLib.EnumLogContentType.Info, UserManager.Instance.CurrentUserName);
                    }
                    else
                    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            LogRecorder.RecordUserOperationLog($"保存用户权限", WestDragon.Framework.BaseLoggerClsLib.EnumLogContentType.Info, UserManager.Instance.CurrentUserName);
        }
