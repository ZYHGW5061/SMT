...
{
    public partial class ComponentMapStep_ColumnParam : ComponentMapStepBasePage
    {
        ...
        public ComponentMapStep_ColumnParam()
        {
            InitializeComponent();
        }
        ...
        {
            get
            { return EnumDefineSetupRecipeComponentMapStep.SetColumnMap; }
        }
        ...
        public override void NotifyStepFinished(out bool finished, out EnumDefineSetupRecipeComponentMapStep currentStep)
        {
            try
            {
                currentStep = EnumDefineSetupRecipeComponentMapStep.SetColumnMap;
                var usedCamera = EditRecipe.CurrentComponent.PositionComponentVisionParameters.VisionPositionUsedCamera;
                var posX = (float)(usedCamera == EnumCameraType.BondCamera ? _positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.BondX)
                    : _positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.WaferTableX));
                var posY = (float)(usedCamera == EnumCameraType.BondCamera ? _positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.BondY)
                    : _positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.WaferTableY));
                LastColumnComponentPosition = new PointF(posX, posY);
                finished = true;
            }
            finally
            {
