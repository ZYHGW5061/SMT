...
{
    public partial class ComponentCalibrationAfterPPStep_SizeRUCorner : ComponentCalibrationAfterPPStepBasePage
    {
        ...
        public ComponentCalibrationAfterPPStep_SizeRUCorner()
        {
            InitializeComponent();
        }
        ...
        {
            get
            { return EnumDefineSetupRecipeComponentCalibrationAfterPPStep.SetComponentRightUpperCorner; }
        }
        ...
        public override void LoadEditedRecipe(BondRecipe recipe)
        {
            try
            {
                if (recipe == null)
                {
                    throw new Exception("Recipe is null when execute ComponentCalibrationAfterPPStep_SizeLUCorner LoadEditedRecipe.");
                }
                EditRecipe = recipe;
        ...
        public override void NotifyStepFinished(out bool finished, out EnumDefineSetupRecipeComponentCalibrationAfterPPStep currentStep)
        {
            try
            {
                currentStep = EnumDefineSetupRecipeComponentCalibrationAfterPPStep.SetComponentRightUpperCorner;
                if (EditRecipe.CurrentComponent.CalibrationAfterPPComponentPositionVisionParameters.VisionPositionUsedCamera == EnumCameraType.BondCamera)
                {
                    RightUpperCornerCoor = new PointF((float)_positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.BondX), (float)_positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.BondY));
                }
                else if (EditRecipe.CurrentComponent.CalibrationAfterPPComponentPositionVisionParameters.VisionPositionUsedCamera == EnumCameraType.WaferCamera)
                {
                    RightUpperCornerCoor = new PointF((float)_positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.WaferTableX), (float)_positioningSystem.ReadCurrentSystemPosition(EnumStageAxis.WaferTableY));
                }
                finished = true;
            }
            finally
