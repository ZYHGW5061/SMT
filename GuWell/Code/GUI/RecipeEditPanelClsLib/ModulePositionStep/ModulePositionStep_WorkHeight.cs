...
{
    public partial class ModulePositionStep_WorkHeight : ModulePositionStepBasePage
    {
        private PackagedLightController _packagedLightController;
        public ModulePositionStep_WorkHeight()
        {
            InitializeComponent();
        }
        ...
        { 
            get 
            { return EnumDefineSetupRecipeModulePositionStep.SetWorkHeight; } 
        }
        /// <summary>
        /// 系统配置
        /// </summary>
        ...
        public override void NotifyStepFinished(out bool finished, out EnumDefineSetupRecipeModulePositionStep currentStep)
        {
            try
            {
                currentStep = EnumDefineSetupRecipeModulePositionStep.SetWorkHeight;
                //PPWorkHeight = 0f;
                finished = true;
            }
            finally
        ...
        private void btnLaserMeasureHeight_Click(object sender, EventArgs e)
        {
            ...
            if (WarningBox.FormShow("即将自动测量Module工作高度。", "确认榜头相机已定位到Module上方！", "提示") == 1)
            {
                try
                {
                    CreateWaitDialog();
                    ...
                    ModuleTopplateHigherValueThanMarkTopplate = (float)-componentZ;
                    _positioningSystem.MoveAixsToStageCoord(EnumStageAxis.BondX, -_systemConfig.PositioningConfig.LaserSensorAndBondCameraOffset.X,
                        EnumCoordSetType.Relative);
                    _positioningSystem.MoveAixsToStageCoord(EnumStageAxis.BondY, -_systemConfig.PositioningConfig.LaserSensorAndBondCameraOffset.Y,
                        EnumCoordSetType.Relative);
