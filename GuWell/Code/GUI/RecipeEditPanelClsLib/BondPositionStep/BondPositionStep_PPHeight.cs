...
{
    public partial class BondPositionStep_PPHeight : BondPositionStepBasePage
    {
        private PackagedLightController _packagedLightController;
        public BondPositionStep_PPHeight()
        {
            InitializeComponent();
        }
        /// <summary>
        ...
        { 
            get 
            { return EnumDefineSetupRecipeBondPositionStep.SetPPHeight; } 
        }
        public override void LoadEditedRecipe(BondRecipe recipe)
        {
            try
            {
                ...
                ckbMeasureEveryHeightWithLaser.Checked = EditRecipe.CurrentBondPosition.MeasureEveryHeightWithLaser;
            }
            catch (Exception ex)
            {
        ...
        public override void NotifyStepFinished(out bool finished, out EnumDefineSetupRecipeBondPositionStep currentStep)
        {
            try
            {
                currentStep = EnumDefineSetupRecipeBondPositionStep.SetPPHeight;
                EditRecipe.CurrentBondPosition.MeasureEveryHeightWithLaser = ckbMeasureEveryHeightWithLaser.Checked;
                //PPWorkHeight = 0f;
                finished = true;
            }
            finally
        ...
        {
            if (WarningBox.FormShow("即将自动测量贴装位置工作高度。", "确认榜头相机已定位到贴装位置上方？", "提示") == 1)
            {
                try
                {
                    CreateWaitDialog();
